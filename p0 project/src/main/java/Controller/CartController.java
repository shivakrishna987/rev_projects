...
public class CartController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		System.out.println(req.getRequestURI().substring("/EcommerceWeb/Carts/".length()+1));

        if (req.getRequestURI().substring("/EcommerceWeb/Carts/".length()+1).equals("all")) {
            try {
    		...
    		Long id=Long.parseLong(req.getRequestURI().substring("/EcommerceWeb/Carts/".length()));
