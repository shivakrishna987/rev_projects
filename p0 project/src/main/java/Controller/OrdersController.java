...
public class OrdersController {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(req.getRequestURI().substring("/EcommerceWeb/Orders/".length()).equals("all")) {
			try {
				...
				for(OrdersResponse order:orders) {
                    ...
                    out.printf("<p>Product ID: %d</p>", order.getProductID());
                    out.printf("<p>Seller ID: %d</p>", order.getSellerID());
				}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException, OrderCreateException {
		...
		OrdersRequest orderRequest= new OrdersRequest(Order_id, user_id, ShippingAddress, BillingAddress, Order_Status, ProductID, SellerID);
