...
public class OrdersDAOClass implements OrdersDAO{
	...
	public boolean CreateOrder(OrdersRequest orderRequest) throws SQLException, OrderCreateException {
		...
		String query="INSERT INTO Orders(Order_id, user_id, ShippingAddress, BillingAddress,Order_Status, ProductID,SellerID) VALUES(?,?,?,?,?,?,?)";
		...
		try(Connection con=ConnectionFactory.getConnectionFactory().getConnection()){
			...
			stmt.setInt(7, orderRequest.getSellerID());
...
//            stmt.setString(6, orderRequest.getOrder_Status());
	...
	public boolean UpateOrder(OrdersRequest orderRequest) throws SQLException {
		...
		String query="UPDATE Orders SET user_id=?, ShippingAddress=?, BillingAddress=?, Order_Status=? WHERE Order_id=?";
		try(Connection con=ConnectionFactory.getConnectionFactory().getConnection()){
			PreparedStatement stmt=con.prepareStatement(query);
...
//            stmt.setTimestamp(4, timestamp);
            stmt.setString(4, orderRequest.getOrder_Status());
            stmt.setInt(5, orderRequest.getOrder_id());
            stmt.setInt(6, orderRequest.getSellerID());
	...
	public List<OrdersResponse> getAllOrders() throws SQLException, OrderNotFoundException {
	 ...
	 public List<OrdersResponse>getOrdersBySellerId(long sellerId) throws SQLException {
	        logger.debug("Entering into getOrdersBySellerId method with SellerID: {}", sellerId);
	        ...
	        try (Connection con = ConnectionFactory.getConnectionFactory().getConnection()) {
	            ...
	            try (ResultSet rs = stmt.executeQuery()) {
	                ...
	                logger.info("Orders fetched successfully for SellerID: {}", sellerId);
	            }
	        } catch (SQLException e) {
	            logger.error("SQL Error occurred while fetching orders for SellerID: {}", sellerId);
	            e.printStackTrace();
	        }
	        return orderList;
	    }
